...
public struct CreativeResponseModel<T: Codable>: Codable {
	...
	public let linkBehavior: LinkOpenTarget?
	public let children: [T]

	public init(responseKey: String, styles: LayoutStyle<CreativeResponseElements>?, linkBehavior: LinkOpenTarget?, children: [T]) {
...
public struct RichTextModel: Codable {
	...
	public init(styles: LayoutStyle<RichTextElements>?, linkBehavior: LinkOpenTarget?, value: String) {
		self.styles = styles
		self.linkBehavior = linkBehavior
		self.value = value
	}
...
public indirect enum LayoutSchemaModel: Codable {
	...
	case bottomSheet(BottomSheetModel<LayoutSchemaModel>)
	case when(WhenModel<LayoutSchemaModel>)
	case staticLink(StaticLinkModel<LayoutSchemaModel>)
	case closeButton(CloseButtonModel<LayoutSchemaModel>)
	...
	public init(from decoder: Decoder) throws {
		...
		if let type = try? container.decode(CodingKeys.self, forKey: .type) {
			...
			case .bottomSheet:
				if let content = try? container.decode(BottomSheetModel<LayoutSchemaModel>.self, forKey: .node) {
			...
			case .staticLink:
				if let content = try? container.decode(StaticLinkModel<LayoutSchemaModel>.self, forKey: .node) {
					self = .staticLink(content)
					return
				}
			case .closeButton:
				if let content = try? container.decode(CloseButtonModel<LayoutSchemaModel>.self, forKey: .node) {
